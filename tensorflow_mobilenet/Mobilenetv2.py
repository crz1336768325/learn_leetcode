...
class Mobilenetv2():
    ...
    def __build(self):
        ...
        with tf.variable_scope('zsc_feature'):
            #none,none,none,3
            x = PrimaryConv('PrimaryConv',self.input_image,32,self.norm,self.activate,self.is_training)
            #skip_0 = x
        ...
        if Detection_or_Classifier=='classifier':
            with tf.variable_scope('zsc_classifier'):
                x = _conv_block('FinalConv',x,self.num_classes,1,1,'SAME',self.norm,self.activate,self.is_training)
...
################################################################################################################
##Mobilenetv2Block
def Mobilenetv2Block(name,x,ratio=6,num_filters=16,repeat=1,stride=1,norm='batch_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        if stride==1:
            for i in range(repeat):
                x = DepthwiseBlock('depthwiseblock_{}'.format(i),x,ratio,num_filters,norm,activate,is_training)
        else:
            x = Transition('Transition',x,ratio,num_filters,norm,activate,is_training)
        
        return x
def DepthwiseBlock(name,x,ratio,num_filters=16,norm='batch_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        input = x
        ...
        x = _conv_block('conv_1',x,num_filters,1,1,'SAME',norm,None,is_training)
        ...
        return x
def Transition(name,x,ratio=6,num_filters=16,norm='batch_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        x = _conv_block('conv_0',x,ratio*num_filters,1,1,'SAME',norm,activate,is_training)
        ...
        return x
##primary_conv
def PrimaryConv(name,x,num_filters=32,norm='batch_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        #none,none,none,3
        x = _conv_block('conv_0',x,num_filters,3,2,'SAME',norm,activate,is_training)#none,none/2,none/2,num_filters
        return x
##_conv_block
def _conv_block(name,x,num_filters=16,kernel_size=3,stride=2,padding='SAME',norm='batch_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        ...
        if norm=='batch_norm':
            ...
            x = bn(x, is_training, name='batchnorm')
        elif norm=='bias':
        ...
        return x
##_depthwise_conv2d
def _depthwise_conv2d(name,x,scale=1,kernel_size=3,stride=1,padding='SAME',norm='batch_norm',activate='selu',is_training=True):
