...
class EDSR(object):
	...
	def __init__(self,img_size=32,num_layers=32,feature_size=256,scale=2,output_channels=3):
		...
		self.input = x = tf.placeholder(tf.float32,[None,None,None,output_channels],name='crzin')
	...
	def resume(self,savedir='saved_models'):
		...
		print("Restored!")	
	def resume_pb(self,savedir='saved_models'):
		print("Restoring...")
		self.saver.restore(self.sess,tf.train.latest_checkpoint(savedir))
		print("Restored!")	
		output_graph_def = graph_util.convert_variables_to_constants(self.sess, self.sess.graph.as_graph_def(), ['crzout'])	
		with open("saved_models/subcrz"+'.pb',"wb") as f:
